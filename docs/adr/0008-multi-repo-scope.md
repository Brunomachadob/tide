# ADR-0008: Multi-repo scope and task discovery
Date: 2026-04-02
Status: Accepted

## Context

ADR-0006 established that task files live in `<repo>/.tide/` and are discovered globally via `com.tide.*.plist` files, each carrying a `TIDE_TASK_FILE` pointer back to its source `.md`. This works well for running tasks, but two problems emerged:
...
- Bare `.tide/` directories outside of any git repo (e.g. `~/n26/git/.tide/`)
