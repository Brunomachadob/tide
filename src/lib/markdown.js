...
// Supports: headers, bold, italic, inline code, fenced code blocks, blockquotes, bullet lists.
...
export function renderMarkdown(text) {
  ...
  text = text.split('\n').map(line => {
    ...
    if (/^> /.test(line))    return `${DIM}${CYAN}│${RESET}${DIM} ${line.slice(2)}${RESET}`
    if (/^[-*] /.test(line)) return `  • ${line.slice(2)}`
    return line
  }).join('\n')

  // Inline transforms (bold before italic to handle **bold** vs *italic* correctly)
