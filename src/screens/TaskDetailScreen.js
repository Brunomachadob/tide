...
import { setEnabled, performDeleteTask, taskDir } from '../lib/tasks.js'
...
export default function TaskDetailScreen({ taskId, navigate, goBack, repoRoot, height, tasks, loading, refresh, intervalMs, settings, breadcrumb }) {
  ...
  const handleConfirm = useCallback(() => {
    ...
    if (action === 'kill') {
      ...
      performDeleteTask(taskId, task?.sourcePath)
    }, true)
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      task.profileKey
        ? React.createElement(Field, { label: 'Profile', value: task.profileKey, valueColor: 'green' })
        : null,
      task.profile?.agent
        ? React.createElement(Field, { label: 'Agent', value: task.profile.agent })
        : null,
      task.profile?.model
        ? React.createElement(Field, { label: 'Model', value: task.profile.model })
        : null,
